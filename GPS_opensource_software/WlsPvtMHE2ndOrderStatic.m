function [xHat_k,z,svPos,H,Wpr_rr] = WlsPvtMHE2ndOrderStatic(prs_stack,gpsEph_stack,xo,iono)
...
% Initialize the states at k epoch by xo
xyz0 = xo(1:3);
bc = xo(4);
fc = xo(5);
X_k = [xyz0(1),xyz0(2),xyz0(3),bc,fc]'; % State at the kth epoch
...
while norm(dx) > GnssThresholds.MAXDELPOSFORNAVM
    ...
    for j=L:-1:1
        ...
        for i = 1:numVal
            ...
            C(2*i-1:2*i,:)=[v(1,i), v(2,i), v(3,i), 1, 0;
                            0,      0,      0     , 0, 1];
            ...
            b(2*i,1) = prs(i,jPrr) + GpsConstants.LIGHTSPEED*dtsvDot(i) + rrMps(i) - fc_k;
    ...
    % State Update 
    xHat = xHat+dx;% delta[x,y,z,bc,fc]'
    xyz0 = xyz0(:) + dx(1:3);
    bc = bc + dx(4);
    fc = fc + dx(5);
    
    X_k = [xyz0(1),xyz0(2),xyz0(3),bc,fc]'; % Estimated State at the kth epoch
end
