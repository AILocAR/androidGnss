function gpsPvt = GpsWlsPvtEKFStatic(gnssMeas, allGpsEph, iono, Flag_discon, bRaw)
...
xo_MHE = [-1509706.868, 6195107.314,149731.63, 0, 0]';%initial state for MHE: [center of the Earth, bc=0, fc = 0]'
...
% Outputs of EKF
gpsPvt.allXp           = zeros(N,5)+NaN;
gpsPvt.allPp           = zeros(5,5,N)+NaN;
gpsPvt.allXhat         = zeros(N,5)+NaN;
gpsPvt.allPhat         = zeros(5,5,N)+NaN;
gpsPvt.allXhatS        = zeros(N,5)+NaN;
gpsPvt.allPhatS        = zeros(5,5,N)+NaN;
...
for i=offset:N    
    ...
    if Flag_discon(i) == true
        ...
        xo_MHE = [-1509706.868, 6195107.314,149731.63,0,0]';
    ...
    if count1 > 0
        ...
        if (count1 == 1 && fstop == true) || (Flag_discon(i) == true)
            ...
            fstart = false;
            fhold = false;
            Xp = [xo(1),xo(2),xo(3),xo(4),xo(8)];
            Pp = zeros(5,5);
            Xhat = [xo(1),xo(2),xo(3),xo(4),xo(8)];
            Phat = zeros(5,5);
        elseif count1 == 2 && fwarmup == true
            ...
            [Xp, Pp] = PredictionEKFStatic(Xhat, Phat, gnssMeas, gpsPvt, i, count1 == 2 && fwarmup == true);
            [Xhat,Phat] = AdjustmentEKFStatic(Xp, Pp, prs,gpsEph,iono);
        ...
        gpsPvt.allBcMetersEKF(i) = Xhat(4);
        gpsPvt.allfcMpsEKF(i) = Xhat(5);
        
        % Pack velocity results of EKF     
        gpsPvt.allVelMpsEKF(i,:) = [0,0,0];%velocity in xyz , static user
   ...
   elseif (count0 >= th1 && fhold == true)||(count1 == 0 && fstop == true)||(count1 == 0 && fwarmup == true) || (Flag_discon(i) == true)
    ...
    elseif (count1 == 0 && fstart == true)||(count0 < th1 && fhold == true)
        ...
        [Xhat, Phat] = PredictionEKFStatic(Xhat, Phat, gnssMeas, gpsPvt, i, count1 == 2 && fwarmup == true);
        ...
        gpsPvt.allXyzMMMEKF(i,:) = [Xhat(1),Xhat(2),Xhat(3)];
        gpsPvt.allXyzMMM(i,:) = [Xhat(1),Xhat(2),Xhat(3)];
        ...
        gpsPvt.allBcMetersEKF(i) = Xhat(4);
        gpsPvt.allBcMeters(i) = Xhat(4);
        
        gpsPvt.allfcMpsEKF(i) = Xhat(5);
        gpsPvt.allBcDotMps(i) = Xhat(5);
        
        gpsPvt.allVelMpsEKF(i,:) = [0,0,0];
    ...
    if count_conti == 1 && count1 == 0
        ...
        WindowSize = 24;        
        [xHat_MHE,Sum_numSvs] = MHEstimatorStatic(i,gnssMeas,allGpsEph,WindowSize,xo_MHE,weekNum,count_conti,iono);
...
%     iValid = find(isfinite(gnssMeas.PrM(i,:)) & gnssMeas.Cn0DbHz(i,:) > 0); %index into valid svid
    ...
    elseif (count1 == 1) || (flag_dis == true)
        ...
        % Convert EKF positioning results from ECEF to lla
        llaDegDegMEKFS = Xyz2Lla([gpsPvt.allXhatS(i,1),gpsPvt.allXhatS(i,2),gpsPvt.allXhatS(i,3)]);
        
        % Pack lla positioning results of EKF        
        gpsPvt.allLlaDegDegMEKFS(i,:) = llaDegDegMEKFS;
        
        % Pack ECEF positioning results of EKF
        gpsPvt.allXyzMMMEKFS(i,:) = [gpsPvt.allXhatS(i,1),gpsPvt.allXhatS(i,2),gpsPvt.allXhatS(i,3)];
                
        % Pack timing results of EKF
        gpsPvt.allBcMetersEKFS(i) = gpsPvt.allXhatS(i, 4);
        gpsPvt.allfcMpsEKFS(i) = gpsPvt.allXhatS(i, 5);
        
        % Pack velocity results of EKF     
        gpsPvt.allVelMpsEKFS(i,:) = [0,0,0];%velocity in xyz 
    ...
    elseif count1 >= 2 
        ...
        Xp = gpsPvt.allXp(i+1, :);
        Pp = gpsPvt.allPp(:,:,i+1);
        [Xs, Ps] = SmoothingKFStatic(Xp', Pp, Xhat', Phat, Xs', Ps,i,gnssMeas);
        gpsPvt.allXhatS(i, :) = Xs;
        gpsPvt.allPhatS(:,:,i) = Ps;
        
        % Convert EKF positioning results from ECEF to lla
        llaDegDegMEKFS = Xyz2Lla([Xs(1),Xs(2),Xs(3)]);
        
        % Pack lla positioning results of EKF        
        gpsPvt.allLlaDegDegMEKFS(i,:) = llaDegDegMEKFS;
        
        % Pack ECEF positioning results of EKF
        gpsPvt.allXyzMMMEKFS(i,:) = [Xs(1),Xs(2),Xs(3)];
        ...
        % Pack timing results of EKF
        gpsPvt.allBcMetersEKFS(i) = Xs(4);
        gpsPvt.allfcMpsEKFS(i) = Xs(5);
